static void morse_flipper_rf_rx_edge(void* ctx, bool level, uint16_t duration_ms) {
    ...
    app->rf_edge_n++;
    if(app->screen == MorseFlipperScreenRfRx) app->rf_cs = level;
    ...
    morse_flipper_dec_drain(&app->rf_decoder, app->rf_rx_text, sizeof(app->rf_rx_text));
#endif
}
...
static void morse_flipper_tick_live_rf(MorseFlipperApp* app, uint32_t now_ms) {
    ...
    if(app->screen == MorseFlipperScreenRfRx) {
        ...
        old_peak_dbm = app->rf_peak_dbm;
        old_activity = app->rf_act;
        old_carrier = app->rf_cs;
        old_monitor_tone = app->rf_mon_tone;
        morse_flipper_radio_sync_live(
            &app->radio,
            morse_flipper_rf_frequency_hz(&app->rf),
            true,
        ...
        app->rf_cs = carrier_now;
        thr_dbm = morse_flipper_rf_clamp_dbm(app->rf_mon_thr_dbm);
        app->rf_rssi_sum_dbm += dbm;
        app->rf_rssi_n++;

        if(app->rf_rssi_next_at == 0U) app->rf_rssi_next_at = now_ms + MORSE_FLIPPER_RF_RSSI_WINDOW_MS;
        if(now_ms >= app->rf_rssi_next_at && app->rf_rssi_n != 0U) {
            avg_sum = app->rf_rssi_sum_dbm;
            avg_samples = app->rf_rssi_n;
            ...
            app->rf_act = app->rf_edge_n;
            app->rf_rssi_sum_dbm = 0;
            app->rf_rssi_n = 0U;
            app->rf_edge_n = 0U;
            app->rf_rssi_next_at = now_ms + MORSE_FLIPPER_RF_RSSI_WINDOW_MS;

            if(!app->rf_rssi_valid || app->rf_rssi_dbm > app->rf_peak_dbm) {
                app->rf_peak_dbm = app->rf_rssi_dbm;
                app->rf_rssi_peak_decay_at = now_ms + MORSE_FLIPPER_RF_RSSI_PEAK_DECAY_MS;
            } else if(now_ms >= app->rf_rssi_peak_decay_at && app->rf_peak_dbm > app->rf_rssi_dbm) {
                app->rf_peak_dbm--;
                app->rf_rssi_peak_decay_at = now_ms + MORSE_FLIPPER_RF_RSSI_PEAK_DECAY_MS;
            }
        ...
        if(!old_valid || old_dbm != app->rf_rssi_dbm || old_peak_dbm != app->rf_peak_dbm ||
           old_activity != app->rf_act || old_carrier != app->rf_cs ||
           old_monitor_tone != app->rf_mon_tone) {
            if(old_monitor_tone != app->rf_mon_tone) {
    ...
    if(!app->radio.tx_on && morse_flipper_any_active_notes(app)) {
        ...
        morse_flipper_radio_set_tx_level(&app->radio, false);
        if(app->rf_tail_at != 0U && now_ms >= app->rf_tail_at) {
            morse_flipper_radio_sync_live(
                &app->radio,
                morse_flipper_rf_frequency_hz(&app->rf),
                false,
